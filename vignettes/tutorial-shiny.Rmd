...
\[S_1(iso_i,iso_j|c_1,c_2)=|p(iso_i>iso_j|c_1)+p(iso_i<iso_j|c_2)-1|,\]
where $p(iso_i>iso_j|c_1)$ and $p(iso_i<iso_j|c_2)$ are the frequencies/probabilities that the samples of one isoform is greater or less than in the other in corresponding conditions. Score 2 is defined as
\[S_2(iso_i,iso_j|c_1,c_2)=|mean.dist(iso_i,iso_j|c_1)|+|mean.dist(iso_i,iso_j|c_2)|,\]
where $mean.dist(iso_i,iso_j|c_1)$ and $mean.dist(iso_i,iso_j|c_2)$ are the mean distances of samples in conditions $c_1$ and $c_2$, respectively.
